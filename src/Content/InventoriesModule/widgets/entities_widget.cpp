...
#include "Content/InventoriesModule/widgets/entty_entry_widget.h"
...
QString EntitiesWidget::getInventoryDisplayName(const QString& inventoryId) const {
	auto inventory = d->itemsService->item(QUuid::fromString(inventoryId));
	if (!inventory || inventory->entity->type != ItemType::Container) {
		return QString("Это не инвентарь");
	}

	return inventory->entity->name;
}
